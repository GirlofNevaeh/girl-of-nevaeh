...
function lockPiece(g: Game): number {
  if (!g.piece) return 0;
...
export function hardDrop(g: Game) {
  if (g.over || !g.piece) return 0;
  const now = performance.now();
  if (now - lastHard < 220) return 0;
  lastHard = now;
  let dist = 0;
...
export function tick(g: Game, dt: number, soft: boolean): { locked: boolean; lines: number } {
  if (g.over || !g.piece) return { locked: false, lines: 0 };
  const down = { ...g.piece, y: g.piece.y + 1 };
  const grounded = !free(g.grid, down);
  if (grounded) {
    g.lock += dt;
    if (g.lock >= LOCK[g.diff]) return { locked: true, lines: lockPiece(g) };
    return { locked: false, lines: 0 };
  }
  g.fall += dt * (soft ? 14 : 1);
...
export function previewCells(kind: Kind): [number, number][] {
  return SHAPES[kind][0]!.map(([y, x]) => [y, x] as [number, number]);
}
