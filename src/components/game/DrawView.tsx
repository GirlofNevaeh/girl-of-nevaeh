...
export function DrawView() {
  ...
  return (
    ...
    >
      ...
      >
      <header className="grid shrink-0 grid-cols-4 items-center gap-2 px-3 py-2 pt-[max(0.6rem,env(safe-area-inset-top))]">
        <Button variant="quiet" className="h-10 w-full whitespace-nowrap px-1 text-xs" onClick={() => useGame.getState().backToTitle()}>
          Main Menu
        </Button>
        <h1 className="text-center font-display text-2xl leading-none">Paint</h1>
        <div className="flex justify-center">
          <SoundToggle />
        </div>
        <button
          type="button"
          className="h-10 w-full rounded-[12px] border border-[#ff4ae0] bg-[#ff2bd6] text-xs text-ink"
          onClick={async () => {
            const canvas = canvasRef.current;
