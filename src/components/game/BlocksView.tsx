...
import { playChime, playClick, playDrop, playLine, playTurn, setAmbient, unlockAudio } from "@/game/audio";
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (!g.over) {
        ...
        const ev = tick(g, dt, held.has("ArrowDown") || held.has("KeyS"));
        if (ev.locked) playDrop();
        if (ev.lines > 0) {
          playLine();
          flash = 0.22;
        }
  ...
  return (
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment">
      ...
      <header className="relative z-10 flex items-center gap-3 px-3 py-3 pt-[max(0.6rem,env(safe-area-inset-top))]">
        ...
        <SoundToggle />
        <img src={playPortrait(who)} alt="" className={`h-12 w-10 rounded-[8px] ${portraitFit(who)}`} />
      ...
      <div className="relative z-10 mx-auto flex min-h-0 w-full max-w-lg flex-1 items-start justify-center gap-3 px-2">
        ...
        <aside className="flex w-16 flex-col gap-2">
          {diff !== "hard" ? (
            ...
            <p className="text-[10px] tracking-[0.14em] text-gold uppercase">Hard</p>
          )}
          <p className="mt-2 text-xs text-silver">Lv {g.level}</p>
          <p className="text-xs text-silver">{g.lines} lines</p>
        </aside>
      </div>
      <PadDock className="relative z-10 mx-3 mb-[max(0.6rem,env(safe-area-inset-bottom))] flex items-center justify-between gap-2 lg:hidden">
        <NeonAct
          label="Down"
          kind="pink"
          mark="Down"
          onDown={() => {
            const lines = hardDrop(game.current);
            playDrop();
            if (lines > 0) playLine();
          }}
        />
...
function Mini({ kind }: { kind: string | null }) {
  ...
  const cells = previewCells(kind as "I" | "O" | "T" | "S" | "Z" | "J" | "L");
  const pink = COLOR[kind as keyof typeof COLOR] === 1;
  const color = pink ? PINK : BLUE;
  return (
    <div className="grid h-14 w-14 grid-cols-4 grid-rows-4 gap-[2px] rounded-[8px] border border-white/10 bg-ink/70 p-1">
      {Array.from({ length: 16 }, (_, i) => {
        const r = Math.floor(i / 4);
        const c = i % 4;
        const on = cells.some(([y, x]) => y === r && x === c);
        return (
          <span
            key={i}
            className="block rounded-[2px]"
            style={{ background: on ? color : "transparent", boxShadow: on ? `0 0 6px ${color}` : undefined }}
          />
        );
      })}
    </div>
...
function draw(ctx: CanvasRenderingContext2D, g: Game, flash = 0) {
  ...
  if (flash > 0) {
    ctx.fillStyle = `rgba(60,240,255,${Math.min(0.35, flash * 1.6)})`;
    ctx.fillRect(0, 0, W, H);
  }
}
