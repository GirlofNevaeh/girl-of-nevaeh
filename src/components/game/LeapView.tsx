...
function CivilPlay({ who, onPick }: { who: CharacterId; onPick: () => void }) {
  ...
  useEffect(() => {
    ...
    window.__leapTest = {
      skipToBoss: (level = 1) => {
        const next = newLeap(level, { lives: 3, score: world.current.score, power: 0 });
        next.hero = who;
        next.phase = "intro";
        next.lock = 0;
        next.cam = 0;
      ...
      getBoss: () => world.current.foes.find((f) => f.boss) ?? null,
      getWorld: () => ({
        phase: world.current.phase,
        level: world.current.level,
        hp: world.current.hp,
        bossHp: world.current.foes.find((f) => f.boss)?.hp ?? 0,
        bossMax: world.current.foes.find((f) => f.boss)?.max ?? 0,
        specials: world.current.bossSpecials,
      }),
    };
    return () => {
      ...
      if (window.__leapTest) delete window.__leapTest;
    };
...
declare global {
  interface Window {
    __leapTest?: {
      ...
      getBoss: () => Foe | null;
      getWorld: () => { phase: string; level: number; hp: number; bossHp: number; bossMax: number; specials: number };
    };
  }
}
