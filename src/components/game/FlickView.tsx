...
import { H, W, beginBoss, fireOrb, newFlick, stepFlick, type FlickWorld, type FoeKind, type ShotHue } from "@/flick/engine";
import { useEffect, useRef, useState } from "react";
import { RosterPick } from "./RosterPick";
import { Hearts } from "./Hearts";
import { SoundToggle } from "@/components/ui/sound-toggle";
import { SHELL } from "@/game/pin-frame";
...
export function FlickView() {
  ...
  const [hud, setHud] = useState({ score: 0, lives: 3, level: 1, over: false, bossHp: 0, bossMax: 0, bosses: 0 });
  const [hi, setHi] = useState(() => Number(localStorage.getItem(HI_KEY) || 0));

  const faces = useRef<Record<string, HTMLImageElement>>({});

  useEffect(() => {
    preloadShips(["nancy", "zorath", "samael", "harlan", "tux", ...CHARACTERS.map((c) => c.id)]);
    if (!who) {
      unlockAudio();
      setAmbient("flick");
      return;
  ...
  useEffect(() => {
    ...
    keys.current = { up: false, down: false, left: false, right: false, fire: false };
    ...
    const step = (now: number) => {
      ...
      setHud({
        ...
        bosses: w.bosses.length + w.bossQueue,
      });
      raf = requestAnimationFrame(step);
    };
    raf = requestAnimationFrame(step);
    window.__flickTest = {
      skipToBoss: (level?: number) => {
        if (level && level !== world.current.level) {
          world.current = newFlick(level, world.current.score, world.current.lives);
        }
        beginBoss(world.current);
        world.current.banner = 0;
        world.current.nextBossIn = 0;
        stepFlick(world.current, 0.05, keys.current);
      },
      getPhase: () => world.current.phase,
      getBoss: () => world.current.bosses[0] ?? null,
      fire: () => {
        world.current.cool = 0;
        world.current.banner = 0;
        return fireOrb(world.current);
      },
      hues: () => world.current.shots.map((s) => s.hue),
      queue: () => world.current.bossQueue,
    };
    window.__controlsTest = {
      getYaw: () => world.current.x,
      getSpeed: () => (keys.current.left || keys.current.right || keys.current.up || keys.current.down ? 1 : 0),
      setKeys: (codes: string[]) => {
        keys.current.up = codes.includes("KeyW") || codes.includes("ArrowUp");
        keys.current.down = codes.includes("KeyS") || codes.includes("ArrowDown");
        ...
        keys.current.fire = codes.includes("Space") || codes.includes("KeyJ");
      },
    };
  ...
  if (!who) {
    return (
      <RosterPick
        title="Sphalerizer"
        blurb="The sky runs toward you. Steer, fire, and clear the lane. A boss ship waits at the end of every level. Later skies send more than one. Three hearts. Best score keeps."
  ...
  return (
    <div className="flex flex-col overflow-hidden bg-ink text-parchment" style={{ ...SHELL, touchAction: "none" }}>
      <header className="shrink-0 px-3 pb-2 pt-[max(3.4rem,env(safe-area-inset-top))]">
        ...
        <h1 className="mt-1 text-center font-display text-2xl font-semibold leading-none">Sphalerizer</h1>
        <div className="mt-1 flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
          ...
          <p className="whitespace-nowrap text-sm text-silver">
            ...
            {hud.bosses > 1 ? ` · ${hud.bosses} ships` : ""}
          </p>
        </div>
      </header>
      <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center gap-3 px-3">
        <div className="flex w-14 flex-col items-center overflow-hidden rounded-[8px] ring-2 ring-[#ff2bd6]">
          <img src={playPortrait(who)} alt="" className={`h-11 w-14 ${portraitFit(who)}`} />
          <p className="w-full truncate bg-[#120814] px-0.5 text-center text-[9px] leading-4 text-[#12d8ff]">{name}</p>
        </div>
        <canvas
          ref={canvasRef}
          width={W}
          height={H}
          className="max-h-full max-w-full touch-none rounded-[16px] border border-[#3cf0ff]/40"
          style={{ width: "100%", height: "auto", aspectRatio: `${W} / ${H}`, maxHeight: "100%" }}
        />
      </div>
      <div className="flex shrink-0 items-center justify-center px-3 pb-[max(0.8rem,env(safe-area-inset-bottom))] pt-3">
        {hud.over ? (
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => {
                playClick();
                world.current = newFlick();
                setHud({ score: 0, lives: 3, level: 1, over: false, bossHp: 0, bossMax: 0, bosses: 0 });
              }}
            >
              Sphalerizer again
            </Button>
          ...
          <PadDock className="pointer-events-auto flex w-full max-w-xl items-center justify-between gap-6 px-5 py-3">
            ...
            <div className="flex min-w-[5.5rem] flex-1 items-center justify-end pr-2">
              <NeonAct
                label="Fire"
                onDown={() => {
                  keys.current.fire = true;
...
function drawShooter(
  ...
  for (const b of w.bosses) {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(x, b.y, b.r + 10, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
    drawBossShip(ctx, b.who, x, b.y, b.r * 2.55);
    const barW = 96;
    const ratio = Math.max(0, b.hp / b.maxHp);
    ctx.fillStyle = "rgba(10,6,16,0.75)";
    ctx.fillRect(x - barW / 2, b.y - b.r - 22, barW, 8);
    ctx.fillStyle = ratio > 0.4 ? "#ff2a2a" : "#ffe000";
    ctx.fillRect(x - barW / 2, b.y - b.r - 22, barW * ratio, 8);
    ctx.strokeStyle = "#ffe000";
    ctx.strokeRect(x - barW / 2, b.y - b.r - 22, barW, 8);
    ctx.fillStyle = "#ffe000";
    ctx.font = "bold 11px sans-serif";
    ctx.textAlign = "center";
    ctx.fillText(w.bossQueue > 0 ? `BOSS ${b.index + 1}` : "BOSS", x, b.y - b.r - 26);
  }
  ...
  for (const s of w.shots) {
    ...
    const len = s.from === "boss" ? 38 : 42;
    const dir = s.from === "boss" ? -1 : 1;
    ctx.save();
    ctx.shadowColor = pal.glow;
    ctx.shadowBlur = 18;
    ctx.fillStyle = pal.glow;
    ctx.fillRect(dir < 0 ? x - len : x, s.y - 3.5, len, 7);
    ctx.fillStyle = pal.core;
    ctx.fillRect(dir < 0 ? x - len + 2 : x, s.y - 1.5, len, 3);
    ctx.restore();
  }
  ...
  if (w.phase === "wave" && gate > -20 && gate < W + 20) {
  ...
  } else if (w.banner > 0 && w.call === "next") {
    drawNeonBanner(ctx, "NEXT", W / 2, H / 2, "#ffe000", "#ff2a2a", 52);
  } else if (w.banner > 0) {
    drawLevelCall(ctx, w.level, W, H);
  }
}

function beamPaint(hue: ShotHue) {
