...
function RowOnline({
  ...
  const [showWin, setShowWin] = useState(false);
  const awarded = useRef(false);
  const youSide: Side = host ? 1 : 2;
  const end = outcome(board);
  const win = winningLine(board);
  const youName = CHARACTERS.find((c) => c.id === you)?.name ?? "Pink";
  const foeName = CHARACTERS.find((c) => c.id === foe)?.name ?? "Blue";
  const youFace = `/art/outlines/${you}.png?v=line2`;
  const foeFace = foe ? `/art/outlines/${foe}.png?v=line2` : "";
  ...
  useEffect(
    () =>
      p2p.onMessage((_from, data) => {
        const msg = data as { t?: string; col?: number };
        if (msg.t === "drop" && typeof msg.col === "number") {
          const side: Side = host ? 2 : 1;
          setBoard((cur) => {
            const next = drop(cur, msg.col!, side);
            return next ?? cur;
          });
          setTurn((t) => (t === 1 ? 2 : 1));
        }
  ...
  useEffect(() => {
    if (!end || awarded.current || !foe) return;
    awarded.current = true;
    if (end === 1) {
      playFourWin();
      setScore((s) => ({ ...s, [you]: (s[you] ?? 0) + 1 }));
  ...
  const dropCol = (col: number) => {
    if (end || !foeReady || turn !== youSide) return;
    const next = drop(board, col, youSide);
    if (!next) return;
    if (youSide === 1) playPinkCoin();
    else playBlueCoin();
    setBoard(next);
    p2p.send({ t: "drop", col });
    if (!outcome(next)) setTurn(youSide === 1 ? 2 : 1);
  };
  ...
  if (!foeReady) {
    return (
      <div className="relative flex h-svh flex-col overflow-hidden bg-ink p-4 text-parchment">
        ...
        <h1 className="mt-6 font-display text-3xl">Six, Seven</h1>
        <p className="mt-2 text-silver">Waiting for a friend. You are {youName}.</p>
        <p className="mt-6 font-display text-5xl tracking-[0.2em] text-[#12d8ff]">{room.replace(/^r4/i, "")}</p>
  ...
  return (
    <div className="relative flex h-svh max-h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment" style={{ overscrollBehavior: "none" }}>
      ...
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col px-4 pb-6">
        ...
        <p className="mt-2 text-center text-sm text-silver">{turn === youSide && !end ? "Your drop." : end ? "" : `Waiting on ${foeName}.`}</p>
        <div className="mt-3 rounded-[20px] border border-[#3cf0ff]/35 bg-[#12182a] p-3">
          <div className="grid grid-cols-7 gap-1.5">
            {Array.from({ length: COLS }, (_, c) => (
              <button
              ...
              Array.from({ length: COLS }, (_, c) => {
                ...
                return (
                  ...
                  >
                    {face ? (
                      <span className="pointer-events-none absolute inset-[16%] overflow-hidden rounded-full">
                        <img src={face} alt="" draggable={false} className="h-full w-full object-contain object-center" />
                      </span>
                    ) : null}
                  </button>
                );
