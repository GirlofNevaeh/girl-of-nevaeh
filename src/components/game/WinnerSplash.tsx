...
export function WinnerSplash({
  ...
  const fit =
    id === "milo" || id === "tux" || id === "murphy"
      ? "object-cover object-center"
      : animal
        ? "object-contain bg-[#0b0b12]"
  ...
  return (
    <div className="fixed inset-0 z-[90] overflow-hidden bg-ink" style={{ position: "fixed" }}>
      <img src={open} alt="" className={cn("absolute inset-0 h-full w-full", fit)} />
      <img
        src={shut}
        alt=""
        className={cn(
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col items-center px-4 pb-[max(1.8rem,env(safe-area-inset-bottom))] pt-16 text-center">
        ...
        {careerNext ? null : (
          <div className="mt-6 flex gap-3">
            {hideAgain ? null : (
              <Button variant="gold" className="min-h-12 min-w-28" onClick={onAgain}>
                Again
              </Button>
            )}
            <Button variant="ghost" className="min-h-12 min-w-28" onClick={onExit}>
              Lobby
            </Button>
          </div>
        )}
