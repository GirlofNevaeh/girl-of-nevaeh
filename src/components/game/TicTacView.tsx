...
import { playChime, playClick, playHeal, setAmbient, unlockAudio } from "@/game/audio";
...
export function TicTacView() {
  ...
  return (
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment">
      ...
      <header className="relative z-10 shrink-0 px-4 py-4 pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <p className="mt-4 font-display text-sm tracking-[0.22em] text-gold uppercase">3-in-a-row</p>
      ...
      >
        <p className="relative z-10 shrink-0 text-silver">Pick who you are and who they face.</p>
        <DuelPick
          embed
          hero={you}
          foe={foe}
...
function CpuMatch({
  ...
  }, [board, turn, end, cpuSide, youSide, foe, onWin, hot]);

  const tap = (i: number) => {
    if (end || thinking) return;
    if (!hot && turn !== youSide) return;
    const side: Side = hot ? turn : youSide;
    const next = place(board, i, side);
    if (!next) return;
    playClick();
    setBoard(next);
    const o = outcome(next);
  ...
  return (
    <BoardView
      ...
      line={line}
      locked={thinking || (!hot && turn !== youSide)}
      hotSeat={hot}
      score={score}
      status={
        end === youSide
          ...
          : end === cpuSide
            ...
            : end === "draw"
              ...
              : hot
                ? (turn === 1 ? FIGHTERS[you].name : FIGHTERS[foe].name) + " to place."
                : thinking
...
function OnlineMatch({
  ...
  const p2p = useLinkRoom({ room, name: you, host: hostSeat });
  ...
  }, [p2p.send, you, world, foeReady]);

  useEffect(() => {
    if (!end || end === "draw" || awarded.current || !foeId) return;
  ...
  useEffect(
    ...
    [p2p.onMessage, hostSeat, onWorld],
  );
  ...
  const tap = (i: number) => {
    ...
    setBoard(next);
    p2p.send({ t: "move", i });
    if (outcome(next) === "draw") playChime();
    setTurn((t) => (t === 1 ? 2 : 1));
  };
  ...
  return (
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-xl items-center justify-between gap-3 px-4">
        <Seat who={you} side={youSide} active={!end && turn === youSide} score={score[you] ?? 0} label={hotSeat ? "P1" : "You"} />
          ...
          return (
            <button
              ...
              className={cn(
                ...
                win ? "four-win-flash ring-2 ring-white" : "",
...
function Seat({
  ...
  return (
    <div className={cn("flex items-center gap-2 rounded-[14px] border bg-ink/60 px-2 py-1", active ? "border-gold" : "border-gold/20")}>
      <img src={playPortrait(who)} alt="" className={`h-12 w-10 shrink-0 rounded-[8px] ${portraitFit(who)}`} />
