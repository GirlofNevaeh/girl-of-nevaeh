...
const MENU: string[] = ["title", "select", "credits", "codex", "duel", "marks", "chess", "draughts", "blocks", "armada", "smartz", "jumble", "flip", "leap", "devil", "row4", "draw", "flick", "sudoku", "words", "guess", "salad"];
...
export function GameApp() {
  ...
  useEffect(() => {
    ...
    else if (screen === "leap") setAmbient("civil");
    else if (screen === "devil") setAmbient("devil");
    else if (screen === "blocks") setAmbient("blocks");
    else if (screen === "marks") setAmbient("marks");
  ...
  return (
    <div
      className="flex min-h-0 flex-col bg-ink text-parchment select-none"
      style={{
        ...SHELL,
        overflow: screen === "title" ? "auto" : "hidden",
        overscrollBehavior: screen === "title" ? "contain" : "none",
        touchAction: screen === "title" ? "manipulation" : "manipulation",
        WebkitOverflowScrolling: screen === "title" ? "touch" : undefined,
      }}
    ...
    >
      {screen === "title" ? (
        <TitleScreen
          ...
          onSudoku={() => useGame.getState().openSudoku()}
          onWords={() => useGame.getState().openWords()}
          onGuess={() => useGame.getState().openGuess()}
          onSalad={() => useGame.getState().openSalad()}
      ...
      {screen === "sudoku" ? <SudokuView /> : null}
      {screen === "words" ? <WordSearchView /> : null}
      {screen === "guess" ? <GuessView /> : null}
      {screen === "salad" ? <SaladView /> : null}

      {screen === "credits" ? (
        <CreditsScreen onTitle={() => useGame.getState().backToTitle()} />
      ) : null}
