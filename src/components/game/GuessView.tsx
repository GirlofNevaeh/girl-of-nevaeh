...
export function GuessView() {
  ...
  const bumpHi = useCallback((h: Record<Diff, number>) => {
    setHi((p) => (p.easy === h.easy && p.normal === h.normal && p.hard === h.hard ? p : h));
  }, []);

  if (mode === "cpu" && you && foe) {
    return <CpuPlay you={you} foe={foe} diff={diff} hi={hi} onHi={bumpHi} onLobby={() => setMode("pick")} />;
  }
  if (mode === "hot" && you && foe) {
    return <HotPlay you={you} foe={foe} diff={diff} hi={hi} onHi={bumpHi} onLobby={() => setMode("pick")} />;
  }
...
function CpuPlay({
  ...
  useEffect(() => {
    if (!done || !round.solved || saved.current) return;
    saved.current = true;
    onHi(saveHigh(diff, pts));
...
function HotPlay({
  ...
  const [phase, setPhase] = useState<HotPhase>(hotStart);
  ...
  const doneB = guessB.solved || guessB.guesses >= MAX_GUESSES;
  const doneA = guessA.solved || guessA.guesses >= MAX_GUESSES;
  const scoredB = useRef(false);
  const scoredA = useRef(false);

  const seat = hotWho(phase);
  const actor = seat === "a" ? you : seat === "b" ? foe : null;

  useEffect(() => {
    if (phase !== "guessB" || !doneB || scoredB.current) return;
    scoredB.current = true;
    setScoreB(scoreFor(guessB.guesses, guessB.solved, diff));
  }, [phase, doneB, guessB.guesses, guessB.solved, diff]);

  useEffect(() => {
    if (phase !== "guessA" || !doneA || scoredA.current) return;
    scoredA.current = true;
    const pts = scoreFor(guessA.guesses, guessA.solved, diff);
    setScoreA(pts);
    onHi(saveHigh(diff, Math.max(pts, scoreB)));
    setPhase("end");
  }, [phase, doneA, guessA.guesses, guessA.solved, diff, onHi, scoreB]);
  ...
  return (
    <Shell
      ...
      hi={hi[diff]}
      onLobby={onLobby}
      turnName={actor ? FIGHTERS[actor].name : ""}
    >
      {isHandoff(phase) && actor ? (
        <HandOff
          who={actor}
          blurb={
            phase === "handA"
              ? "Pass the phone. Pick a secret number."
              : phase === "handB"
                ? "Pass the phone. Player 2 picks a secret number."
                : "Pass the phone back. Guess their number."
          }
          onGo={() => {
            playClick();
            setPhase(hotAdvance(phase));
          }}
        />
      ) : null}
      ...
      {phase === "guessB" && secretA != null ? (
        <>
          ...
          <GuessBoard round={guessB} tone="blue" disabled={doneB} />
          {doneB ? (
      ...
      {phase === "guessA" && secretB != null ? (
        <>
          ...
          <GuessBoard round={guessA} tone="pink" disabled={doneA} />
        </>
      ) : null}
      {phase === "end" ? (
        <WinnerSplash
          ...
          foeName={FIGHTERS[foe].name}
          onAgain={() => {
            scoredA.current = false;
            scoredB.current = false;
            setPhase(hotStart());
            setSecretA(null);
            setSecretB(null);
            setScoreA(0);
...
function HandOff({ who, blurb, onGo }: { who: FightFighterId; blurb: string; onGo: () => void }) {
  return (
    <div className="mx-auto flex w-full max-w-sm flex-col items-center pt-2">
      <img src={playPortrait(who)} alt="" className={`h-44 w-36 rounded-[18px] ${portraitFit(who)}`} />
      <p className="mt-4 text-center font-display text-3xl">{FIGHTERS[who].name}</p>
      <p className="mt-2 text-center text-sm text-silver">{blurb}</p>
      <Button variant="pink" className="mt-6 min-h-12 w-full" onClick={onGo}>
        I'm ready
      </Button>
    </div>
  );
...
function OnlinePlay({
  ...
  }, [p2p.send, you, diff, foeReady]);

  useEffect(() => {
    if (mine == null || !foeReady) return;
    p2p.send({ t: "secret", n: mine });
  }, [mine, foeReady, p2p.send]);
  ...
  useEffect(
    ...
    [p2p.onMessage, round.reset],
  );

  useEffect(() => {
    if (!iAmDone || !theirs || sentDone.current) return;
  ...
  }, [iAmDone, theirs, round.guesses, round.solved, myPts, p2p.send, onHi, diff]);
    ...
    <Shell you={you} foe={foeId} diff={diff} hi={hi[diff]} onLobby={onLobby} turnName={FIGHTERS[you].name}>
      {!foeReady ? (
        <div className="text-center">
          <p className="font-display text-5xl tracking-[0.2em] text-[#12d8ff]">{room.replace(/^ng/i, "")}</p>
          <p className="mt-2 text-sm text-[#12d8ff]">{p2p.status}</p>
        </div>
      ) : null}
      {foeReady && mine == null ? (
        <SetNumber
      ...
      {foeReady && theirs != null && !bothDone ? (
        <>
          ...
          {iAmDone ? <p className="mt-3 text-center text-sm text-gold">Waiting for {FIGHTERS[foeId].name}…</p> : null}
        </>
      ) : null}
      {bothDone && winId ? (
        <WinnerSplash
          ...
          youName={FIGHTERS[you].name}
          foeName={foeId ? FIGHTERS[foeId].name : "Friend"}
          onAgain={onLobby}
          onExit={onLobby}
        />
      ) : null}
...
function useGuessRound(diff: Diff, start?: number) {
  ...
  const submit = () => {
    if (solved || guesses >= MAX_GUESSES) return;
    const n = numberFromSlots(slots);
    if (n == null) {
      playRefuse();
      setWarn("Enter two digits");
      return;
    }
...
function SetNumber({ who, diff, onSet }: { who: FightFighterId; diff: Diff; onSet: (n: number) => void }) {
  ...
  return (
    <div className="mx-auto w-full max-w-sm">
      <p className="text-center text-sm text-silver">
        {FIGHTERS[who].name}: pick a number from 1 to {RANGE[diff]}. Two digits. For 1–9 start with 0.
      </p>
  ...
  return (
    <div className="mx-auto w-full max-w-sm">
      ...
      <p className="mt-1 text-center text-xs tracking-[0.16em] text-gold uppercase">
        {left} guess{left === 1 ? "" : "es"} left · two digits · 1–{round.max}
      </p>
      ...
      <div className="mt-3 flex justify-center">
        <Button variant={pink ? "pink" : "blue"} disabled={disabled || numberFromSlots(round.slots) == null} onClick={() => round.submit()}>
          Guess
        </Button>
      </div>
      {round.log.length ? (
