  ...
  return (
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment">
      ...
      <header className="relative z-10 shrink-0 px-4 py-3 pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <h1 className="mt-2 font-display text-3xl font-semibold leading-none">Draughts</h1>
    ...
    <DuelPick
      ...
      footer={
        <OnlinePanel
          ...
          onHost={onHost}
          onGuest={onGuest}
          note="One board. Two devices. Host is pink. Guest is blue."
        />
      }
    />
  );
...
function OnlineMatch({
  ...
  useEffect(
    ...
    [p2p.onMessage, foeId, onWin, youSide],
  );
...
function Seat({
  ...
  return (
    <div
      className={cn(
        "flex h-[4.6rem] min-w-0 items-center gap-2 rounded-[14px] border bg-ink/70 px-2 py-1.5",
        active ? ring : "border-gold/20",
      )}
    >
      <img src={playPortrait(who)} alt="" className={`size-12 shrink-0 rounded-[8px] ${portraitFit(who)}`} />
      <div className="min-w-0 flex-1">
        <p className="font-display text-[12px] leading-tight break-words">{f.name}</p>
