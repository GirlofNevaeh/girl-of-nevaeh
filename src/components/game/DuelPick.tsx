...
export function DuelPick({
  ...
  return (
    <div className={cn("mx-auto w-full max-w-5xl py-2", embed ? "" : "min-h-0 flex-1 overflow-y-auto overscroll-contain")}>
      <div className={cn("grid gap-2 items-stretch", showFoe || beside ? "grid-cols-2" : "grid-cols-1")}>
        <SeatCard
          active={!pickingFoe}
          label="You"
          empty="You"
        ...
        {showFoe ? (
          ...
          <div className="min-h-0 overflow-hidden rounded-[18px] border border-[#3cf0ff]/45 bg-ink-soft/90 p-3">
            {beside}
          </div>
        ) : null}
      </div>
      ...
      <div className="roster-grid mt-3 grid grid-cols-3 gap-2 pb-4 sm:grid-cols-4 md:grid-cols-5">
        {PLAYABLE.map((id) => {
...
function SeatCard({
  ...
  return (
    ...
    >
      {id ? (
        <img src={playPortrait(id)} alt="" className={`h-14 w-11 shrink-0 rounded-[10px] ${portraitFit(id)}`} />
      ) : (
