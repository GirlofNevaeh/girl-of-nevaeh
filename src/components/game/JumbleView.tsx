...
export function JumbleView() {
  ...
  return (
    <div className="flex h-svh max-h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment" style={{ overscrollBehavior: "none" }}>
      <header className="flex items-center gap-2 px-3 py-3 pt-[max(1rem,env(safe-area-inset-top))]">
        <Button variant="quiet" onClick={() => useGame.getState().backToTitle()}>
          Main Menu
        </Button>
        <Button variant="quiet" onClick={() => setWho(null)}>
          Lobby
        </Button>
        <SoundToggle />
        <Button
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col px-4">
        ...
        <div className="mt-4 mb-3 flex flex-col items-center gap-1.5">
          <img
            src={art}
            alt=""
            className="h-16 w-16 rounded-full object-cover object-top ring-2 ring-[#3cf0ff]/70"
          />
          <p className="font-display text-lg text-parchment">{face.name}</p>
        </div>
      </main>
    </div>
  );
