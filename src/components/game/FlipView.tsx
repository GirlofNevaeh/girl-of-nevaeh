...
export function FlipView() {
  ...
  return (
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment">
      <header className="flex shrink-0 items-center gap-2 px-3 py-2 pt-[max(0.6rem,env(safe-area-inset-top))]">
        ...
        <SoundToggle />
        <p className="ml-auto text-right text-[11px] text-silver">
          {moves} moves · {score} · best {hi}
        </p>
      </header>
      <main className="flex min-h-0 flex-1 flex-col items-center justify-center px-3 pt-1 pb-[max(4.5rem,env(safe-area-inset-bottom))]">
        <p
          className="mb-5 shrink-0 pb-1 text-center font-display text-3xl font-extrabold leading-none tracking-wide"
          style={{
            color: "#3cf0ff",
            WebkitTextStroke: "3px #ff2bd6",
            paintOrder: "stroke fill",
            textShadow: "0 0 14px #ff2bd6",
          }}
        >
          Level {level + 1}
        </p>
