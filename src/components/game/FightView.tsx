...
import { hushMusic, playChime, playClick, playFightCall, playHeal, playKo, setAmbient, unlockAudio } from "@/game/audio";
...
export function FightView() {
  ...
  const [hostSeat, setHostSeat] = useState(true);
    ...
    <div className="relative flex h-svh flex-col items-center justify-center overflow-hidden bg-ink px-4 text-center" style={{ position: "fixed", inset: 0, overscrollBehavior: "none", touchAction: "none" }}>
      ...
      <div className="mt-8 flex w-full max-w-lg items-center justify-center gap-3">
        <div className="min-w-0 flex-1">
          <img src={playPortrait(hero)} alt="" className={`mx-auto aspect-3/4 w-full max-w-40 rounded-[16px] ${portraitFit(hero)}`} />
          <p className="mt-2 font-display text-lg">{FIGHTERS[hero].name}</p>
        </div>
        <p className="font-display text-2xl text-[#3cf0ff]">V</p>
        <div className="min-w-0 flex-1">
          <img src={playPortrait(foe)} alt="" className={`mx-auto aspect-3/4 w-full max-w-40 rounded-[16px] ${portraitFit(foe)}`} />
    ...
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment" style={{ position: "fixed", inset: 0, overscrollBehavior: "none" }}>
      ...
      <main className="relative z-10 mx-auto flex min-h-0 w-full max-w-5xl flex-1 flex-col overflow-y-auto px-4 py-4" style={{ overscrollBehavior: "none", WebkitOverflowScrolling: "auto" }}>
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            {hero ? (
              <img src={playPortrait(hero)} alt="" className={`h-14 w-11 shrink-0 rounded-[10px] ${portraitFit(hero)}`} />
            ) : (
          ...
          >
            {foe ? (
              <img src={playPortrait(foe)} alt="" className={`h-14 w-11 shrink-0 rounded-[10px] ${portraitFit(foe)}`} />
            ) : (
              <span className="flex h-14 w-11 shrink-0 items-center justify-center rounded-[10px] border border-dashed border-gold/30 text-[10px] text-muted">
                Foe
              </span>
        ...
        <div className="mt-4">
          <OnlinePanel
            ...
            onHost={onHost}
            onGuest={onGuest}
            note="Two devices. Host is the left fighter. Pick your face first."
          />
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
          {HEROES.map((id) => {
            ...
            return (
              ...
              >
                <img src={playPortrait(id)} alt="" className={`aspect-3/4 w-full ${portraitFit(id)}`} />
      ...
      <div className="pointer-events-none absolute h-px w-px overflow-hidden opacity-0" aria-hidden>
        {HEROES.map((id) => (
          <span key={id}>
            ...
            <img src={`/art/fighters/${id}-atk.png?v=atk14`} alt="" />
...
function ArenaNet({
  ...
  const p2p = useLinkRoom({ room, name: you, host });
  const foe = (p2p.peers[0]?.name as FightFighterId) || null;
  const foeReady = !!foe && foe in FIGHTERS;
  useEffect(() => {
    p2p.send({ t: "hello", char: you });
  }, [p2p.send, you, foeReady]);
  if (!foeReady) {
    return (
      <div className="relative flex h-svh flex-col overflow-hidden bg-ink p-4 text-parchment">
        <Button variant="quiet" className="self-start" onClick={onLobby}>
          Lobby
        </Button>
        <h1 className="mt-6 font-display text-3xl">Warrior Battle</h1>
        <p className="mt-2 text-silver">Waiting for a friend. You are {FIGHTERS[you].name}.</p>
        <p className="mt-6 font-display text-5xl tracking-[0.2em] text-[#12d8ff]">{room.replace(/^wb/i, "")}</p>
  ...
  return (
    <Arena
      ...
      rung={0}
      career={false}
      onNextCareer={() => undefined}
      onChangeHero={onLobby}
      onLobby={onLobby}
      net={{ send: p2p.send, onMessage: p2p.onMessage, localSeat: host ? 0 : 1 }}
    />
  );
}
...
function Arena({
  ...
  const remoteIn = useRef<FightInput>({ ...EMPTY });
  ...
  useEffect(() => {
    ...
    return net.onMessage((_from, data) => {
      const msg = data as { t?: string } & Partial<FightInput> & { s?: FightState };
      if (msg.t === "in") {
        remoteIn.current = {
          left: !!msg.left,
          right: !!msg.right,
      ...
      if (msg.t === "snap" && msg.s) {
        ...
        cur.timer = snap.timer;
        cur.round = snap.round;
        cur.banner = snap.banner;
        cur.winner = snap.winner;
        cur.projectiles = snap.projectiles ?? cur.projectiles;
        snap.fighters?.forEach((f, i) => {
          const dst = cur.fighters[i]!;
          Object.assign(dst, f);
        });
      }
    });
  ...
  useEffect(() => {
    ...
    let lastNet = 0;
    let latestInput: FightInput = { ...EMPTY };
    ...
    const loop = (now: number) => {
      ...
      while (acc >= tick) {
        ...
        if (n?.localSeat !== 1) {
          ...
          step(state.current, input, tick, n ? remoteIn.current : undefined);
          remoteIn.current = { ...remoteIn.current, punch: false, kick: false, special: false };
      ...
      if (n && now - lastNet >= 80) {
        lastNet = now;
        if (n.localSeat === 1) n.send({ t: "in", ...latestInput });
        else {
          n.send({
            t: "snap",
            s: {
              ...
              banner: state.current.banner,
              winner: state.current.winner,
              fighters: state.current.fighters,
              projectiles: state.current.projectiles,
            },
          });
        }
      }
  ...
  return (
    ...
    >
      <div className="game-chrome pointer-events-auto relative z-20 flex shrink-0 items-center justify-between gap-2 px-3 py-1.5 pt-[max(0.4rem,env(safe-area-inset-top))]">
        ...
        <SoundToggle />
        <Button variant="quiet" className="px-3" onClick={onChangeHero}>
          Change
        </Button>
      </div>
...
function attackCell(f: Fighter) {
  if (f.attack === "punch") return 0;
  if (f.attack === "special") return 1;
...
function drawFighter(
  ...
  if (f.state === "ko") {
    ctx.globalAlpha = Math.floor(t / 110) % 2 === 0 ? 0.18 : 0.92;
  }
...
function drawHud(ctx: CanvasRenderingContext2D, s: FightState) {
  ...
  bar(ctx, 32, 16, 560, 44, a.hp / a.maxHp, "#ff4ae0", false);
  bar(ctx, 688, 16, 560, 44, b.hp / b.maxHp, "#3cf0ff", true);
  drawHearts(ctx, 540, 108, a.rounds, 2, "right");
  drawHearts(ctx, 720, 108, b.rounds, 2, "left");
  drawFightOrbs(ctx, 32 + 560, 72, a.specials, 3, "left");
  drawFightOrbs(ctx, 688, 72, b.specials, 3, "right");
  ctx.textBaseline = "top";
  ctx.fillStyle = "#f4ead8";
  fitLabel(ctx, a.name.toUpperCase(), 32, 68, "left", 360);
  fitLabel(ctx, b.name.toUpperCase(), 1248, 68, "right", 360);
  ctx.textAlign = "center";
  ctx.fillStyle = "#d4a54a";
  ctx.font = "700 36px 'Cormorant Garamond', serif";
  ctx.textBaseline = "alphabetic";
  ctx.fillText(String(Math.max(0, Math.ceil(s.clock))), W / 2, 48);
}

function drawFightOrbs(ctx: CanvasRenderingContext2D, edge: number, y: number, n: number, max: number, side: "left" | "right") {
  const r = 8;
  const gap = 20;
  for (let i = 0; i < max; i++) {
    const cx = side === "left" ? edge - r - i * gap : edge + r + i * gap;
    const on = i < n;
    ctx.beginPath();
    ctx.arc(cx, y, r, 0, Math.PI * 2);
    ctx.fillStyle = on ? "#12d8ff" : "rgba(18,216,255,0.12)";
    ctx.fill();
...
function drawHearts(ctx: CanvasRenderingContext2D, x: number, y: number, n: number, max: number, align: "left" | "right") {
  const gap = 52;
  for (let i = 0; i < max; i++) {
    const px = align === "left" ? x + i * gap : x - i * gap;
    strokeHeart(ctx, px, y, 24, i < n);
  }
}

function strokeHeart(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, filled: boolean) {
  ctx.save();
  ctx.translate(cx, cy + r * 0.15);
  ctx.beginPath();
  ctx.moveTo(0, r * 0.35);
  ctx.bezierCurveTo(-r * 1.05, -r * 0.25, -r * 0.35, -r * 1.05, 0, -r * 0.38);
  ctx.bezierCurveTo(r * 0.35, -r * 1.05, r * 1.05, -r * 0.25, 0, r * 0.35);
  ctx.closePath();
  if (filled) {
  ...
  ctx.strokeStyle = filled ? "#ffe8ee" : "#f4ead8";
  ctx.lineWidth = 4;
  ctx.stroke();
  ctx.restore();
}
