...
export function CodexApp() {
  ...
  return (
    <div className="relative flex h-svh min-h-0 flex-col overflow-hidden bg-ink text-parchment">
      <header className="relative z-20 grid shrink-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 border-b border-gold/15 bg-ink/90 px-3 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <div className="flex items-center gap-1">
          <Button variant="quiet" className="px-2" onClick={() => useGame.getState().backToTitle()}>
            <ArrowLeft className="size-4" />
            Main Menu
          </Button>
          <Button variant="quiet" className="px-2" aria-label="Orb" onClick={() => useCodex.getState().openOrb()}>
            <Sparkles className="size-4 text-gold" />
          </Button>
        </div>
        <div className="min-w-0 text-center">
          <p className="font-display text-[11px] tracking-[0.18em] text-gold">Girl of NevaeH</p>
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-3">
        <h3 className={cn("font-display text-[12px] font-semibold leading-none tracking-tight whitespace-nowrap sm:text-base", visited && "text-gold")}>{entry.name}</h3>
...
function CreditsLine() {
  return (
    <p className="mt-10 pb-6 text-center text-xs tracking-wide text-faint">
      Based on Girl of NevaeH by R A Simpson
    </p>
  );
}
