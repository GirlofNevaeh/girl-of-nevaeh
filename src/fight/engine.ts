...
> = {
  ...
  ronnie: {
    ...
    body: "/art/fighters/ronnie.png?v=stance4",
    line: "Father",
  },
  veronika: {
    name: "Veronika",
...
export function preloadFightArt() {
  ...
  for (const id of PLAYABLE) {
    ...
    if (!atkCache[id]) atkCache[id] = loadImg(`/art/fighters/${id}-atk.png?v=atk14`);
...
const ATTACK: Record<AttackKind, { frames: number; active: [number, number]; dmg: number; reach: number; h: number; y: number }> =
  {
    ...
    special: { frames: 28, active: [10, 16], dmg: 55, reach: 72, h: 56, y: 180 },
  };
...
function startAttack(f: Fighter, kind: AttackKind) {
  ...
  if (busy(f) && !(f.state === "jump" && kind === "special")) return;
  if (kind === "special" && f.specials <= 0) return;
  if (kind === "special") f.specials -= 1;
  f.state = kind;
  f.attack = kind;
  f.frame = 0;
  f.connected = false;
...
function ai(s: FightState, cpu: Fighter, hero: Fighter, dt: number): FightInput {
  ...
  if (cpu.specials > 0 && dist > 90 && dist < 420 && roll < 0.08 * sharp) {
...
function controlFighter(f: Fighter, input: FightInput, other: Fighter, dt: number) {
  ...
  else if (input.special && f.specials > 0) startAttack(f, "special");
  if (busy(f)) return;
...
export function step(s: FightState, heroIn: FightInput, dt: number, foeIn?: FightInput) {
  ...
  const cpuIn = foeIn ?? ai(s, cpu, hero, dt);
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (f.attack === "special" && f.frame >= 2 && f.frame <= 6) {
      const already = s.projectiles.some((p) => p.from === i && p.who === f.id && p.life > 1.4);
      if (!already) launchSpecial(s, f, i as 0 | 1);
    }
  }
  ...
  for (const p of s.projectiles) {
    ...
    if (overlaps(box, hurtbox(target))) {
      applyHit(s, s.fighters[p.from], target, p.kind === "orb" ? 52 : 46, "special");
