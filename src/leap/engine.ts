...
export function newLeap(level = 1, keep?: { lives: number; score: number; power: number; hp?: number }): LeapWorld {
  ...
  return {
    ...
    roundHold: 0,
    specials: 5,
    bossSpecials: bossSpecialsFor(level),
  };
}
...
export function bossHpFor(level: number, kind?: FoeKind) {
  const who = kind ?? bossKind(level);
  const extra = who === "samael" ? 14 : who === "zorath" ? 8 : who === "karen" ? 0 : 4;
  return 32 + level * 6 + extra;
}

export function bossSpecialsFor(level: number) {
  if (level <= 1) return 0;
  if (level <= 3) return 1;
  if (level <= 5) return 2;
  return 3;
...
function bossWalk(level: number) {
  return 86 + level * 7;
}

function bossEngage(level: number) {
  return 136 + level * 5;
}

function bossStrike(level: number) {
  return 112 + level * 3;
}

function bossWindup(level: number) {
  return Math.max(0.78, 1.16 - level * 0.035);
}

function bossMeleeHurt(level: number) {
  return 6 + level;
}

function bossShotHurt(level: number) {
  return 5 + level;
}

function spawnBoss(w: LeapWorld) {
  const kind = bossKind(w.level);
  const hp = bossHpFor(w.level, kind);
...
export function stepLeap(w: LeapWorld, dt: number, keys: LeapKeys): LeapFx[] {
  ...
  for (const f of w.foes) {
    ...
    const walk = f.boss ? bossWalk(w.level) : 140 + w.level * 12;
    if (gap > (f.boss ? 96 : 70)) f.vx = Math.sign(dx || -1) * walk;
    ...
    if (gap < (f.boss ? bossEngage(w.level) : 160) && f.attackT <= 0) {
      f.attackT = f.boss ? bossWindup(w.level) : 0.55;
      f.struck = false;
    ...
    if (w.attack && ahead && f.hit <= 0 && reach && Math.abs(f.x - w.x) < reach + (f.boss ? 20 : 0) && Math.abs(f.y - w.y) < 110) {
      ...
      !f.struck &&
      f.attackT > 0.08 &&
      f.attackT < (f.boss ? 0.42 : 0.48) &&
      w.invuln <= 0 &&
      gap < (f.boss ? bossStrike(w.level) : 130) &&
      Math.abs(f.y - w.y) < 140
    ) {
      f.struck = true;
      const hurt = f.boss ? bossMeleeHurt(w.level) : 4 + w.level;
      hurtHero(w, hurt, fx);
    }
  }
  ...
  w.shots = w.shots.filter((s) => {
    ...
    if (s.from === "you") {
      ...
      if (hit) {
        hit.hp = hit.boss ? Math.max(0, hit.hp - Math.max(5, Math.ceil(hit.max * 0.12))) : 0;
        hit.hit = 0.16;
    ...
    if (w.invuln <= 0 && Math.abs(s.x - w.x) < 48 && Math.abs(GROUND - 190 - w.y) < 140) {
      hurtHero(w, bossShotHurt(w.level), fx);
      return false;
    }
    return true;
  });
...
function resetBossRing(w: LeapWorld) {
  ...
  w.shots = [];
  w.specials = 5;
  w.bossSpecials = bossSpecialsFor(w.level);
  w.foes = [];
  spawnBoss(w);
}
