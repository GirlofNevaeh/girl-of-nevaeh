...
let bedClip: HTMLAudioElement | null = null;
...
function songWav(key: string, bpm: number, bass: number[], lead: number[], mix = 0.2) {
  const hit = urls.get(key);
  if (hit) return hit;
  const sr = 22050;
  const beat = 60 / bpm;
  const dur = beat * Math.max(bass.length, 8);
  const n = Math.floor(sr * dur);
  const pcm = new Int16Array(n);
  const square = (freq: number, i: number) => (Math.sin((2 * Math.PI * freq * i) / sr) > 0 ? 1 : -1);
  for (let i = 0; i < n; i++) {
    const t = i / sr;
    const leadI = Math.floor(t / (beat * 0.5)) % lead.length;
    const bassI = Math.floor(t / beat) % bass.length;
    const env = 0.5 + 0.5 * Math.sin((Math.PI * (t % (beat * 0.5))) / (beat * 0.5));
    const s =
      square(bass[bassI]!, i) * 0.2 +
      square(lead[leadI]!, i) * mix * env +
      Math.sin((2 * Math.PI * lead[leadI]! * i) / sr) * 0.07 * env;
    pcm[i] = Math.max(-32767, Math.min(32767, s * 22000));
  }
  const url = encodeWav(pcm);
  urls.set(key, url);
  return url;
...
function civilLoopWav() {
  return songWav("civil-loop", 148, [82, 82, 98, 110, 82, 73, 98, 110], [330, 392, 494, 392, 330, 294, 247, 294, 330, 392, 440, 392, 330, 247, 294, 330], 0.16);
}

function fightLoopWav() {
  return songWav("fight-loop", 152, [87, 87, 110, 131, 87, 78, 110, 131], [349, 440, 523, 440, 349, 311, 262, 311, 349, 440, 466, 440, 349, 262, 311, 349], 0.17);
}

function armaLoopWav() {
  return songWav("arma-loop", 118, [65, 65, 98, 65, 73, 73, 98, 110], [262, 330, 262, 196, 262, 294, 330, 196, 247, 262, 196, 165, 196, 247, 262, 196], 0.2);
}

function sphalLoopWav() {
  return songWav("sphal-loop", 136, [73, 98, 110, 98, 82, 110, 123, 110], [392, 494, 587, 523, 392, 349, 330, 349, 440, 523, 587, 440, 392, 330, 294, 330], 0.15);
}

function devilLoopWav() {
  return songWav("devil-loop", 168, [98, 98, 117, 131, 98, 87, 117, 147], [392, 466, 392, 311, 349, 466, 523, 392, 311, 349, 392, 311, 294, 349, 392, 466], 0.18);
}

function blocksLoopWav() {
  return songWav("blocks-loop", 132, [98, 73, 82, 110, 98, 73, 87, 131], [330, 392, 494, 392, 294, 330, 392, 262, 330, 392, 523, 392, 294, 247, 294, 330], 0.16);
}

function xoLoopWav() {
  return songWav("xo-loop", 160, [110, 110, 131, 147, 110, 98, 131, 165], [440, 523, 659, 523, 440, 392, 349, 392, 440, 523, 587, 523, 440, 349, 392, 440], 0.18);
}
...
function startBed(id: AmbientId) {
  ...
  const loops: Partial<Record<AmbientId, () => string>> = {
    civil: civilLoopWav,
    duel: fightLoopWav,
    armada: armaLoopWav,
    flick: sphalLoopWav,
    devil: devilLoopWav,
    blocks: blocksLoopWav,
    marks: xoLoopWav,
  };
  const make = loops[id];
  if (!make) {
  ...
  if (currentAmbient === id && bedClip && !bedClip.paused) return;
  stopBed();
  currentAmbient = id;
  bedClip = new Audio(make());
  bedClip.loop = true;
  bedClip.volume = 0.28;
  void bedClip.play().catch(() => {});
}
...
export function playPowerOrb() {
  if (!enabled) return;
  if (!unlocked) unlockAudio();
  playUrl("power-orb", sweepWav(520, 1240, 0.28, 0.42, "power-orb"), 1);
}
...
export function playLine() {
  playUrl("line", sweepWav(440, 880, 0.28, 0.5, "line"), 0.55);
}
...
export function playPinkCoin() {
  playUrl("pink-coin", toneWav(880, 0.14, 0.45), 0.5);
}

export function playBlueCoin() {
  playUrl("blue-coin", toneWav(523, 0.16, 0.45), 0.5);
}

export function playFourWin() {
  playUrl("four-win", sweepWav(392, 784, 0.45, 0.5, "four-win"), 0.55);
}
